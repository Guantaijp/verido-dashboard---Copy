...
export function DistributorTable<TData extends INewUsers, TValue>({
  columns,
  data,
  isFetching,
  pagination,
            ...
            <DropdownMenuItem
              ...
              onClick={(e) => {
                ...
                navigator.clipboard.writeText(data.referenceId).then(() => {
                  showToast(
                    "Success",
                    `Distributor Id copied to clipboard`,
                    "success"
                  );
                });
              }}
  ...
  const renderContent = () => {
    ...
    return table.getRowModel().rows.map((row) => (
      <TableRow
        ...
        onClick={() => {
          const data = row.original;
          if (hasPermission("distributor.getSingle")) {
            router.push(`/distributors/${data._id}`);
          } else {
            showToast(
              "Access Denied",
              "You do not have permission to access this resource, contact Verido",
    ...
    <div className="rounded-md">
      <div className="flex flex-col md:flex-row  gap-3 md:gap-0 justify-between mb-6">
        <div className="flex flex-col items-start gap-2">
          <h2 className="text-[24px] font-bold">Distrubutors</h2>
        </div>
