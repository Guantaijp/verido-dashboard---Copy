...
import { Copy, MoreVertical, MoveRight, MoveLeft } from "lucide-react";
import BranchForm from "../AllUsers/BranchForm";
...
export function NewBusinessOwnersTable<TData extends INewUsers, TValue>({
  columns,
  data,
  isFetching,
  isBranch,
  ...
  const handleAddBranch = () => {
    if (hasPermission("business_owners.create")) {
  ...
  const renderContent = () => {
      ...
      return (
        <TableRow className="text-sm font-bold text-gray-text">
          <TableCell colSpan={columns.length} className="h-24 text-center">
            {` No ${isBranch ? "Branches" : "Business Owners"} found.`}
          </TableCell>
        </TableRow>
      );
    }
    return table.getRowModel().rows.map((row) => (
      <TableRow
        ...
        onClick={() => {
          const data = row.original;
          if (hasPermission("business_owners.getSingle")) {
            router.push(
              `${
                isBranch
                  ? `/branches/${data._id}`
                  : `/business-owners/${data._id}`
              } `
            );
          } else {
            showToast(
      ...
      <div className="flex flex-col md:flex-row  gap-3 md:gap-0 justify-between mb-6">
        <div className="flex flex-col items-start gap-2">
          <h2 className="text-[24px] font-bold">{`${
            isBranch ? "Branches" : "Business Owners"
          }`}</h2>
        </div>
            ...
            <>
              <Button
                className={`bg-verido-green text-verido-white ${
                  !hasPermission("business_owners.create") && "opacity-50"
                }`}
              ...
              {openFormModal && (
                <Dialog open={openFormModal} onOpenChange={setOpenFormModal}>
                  ...
                  >
                    <BranchForm closeModal={() => setOpenFormModal(false)} />
                  </DialogContent>
                </Dialog>
              )}
            </>
