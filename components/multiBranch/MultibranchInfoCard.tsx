...
interface MultibranchrInfoCardProps {
  multibranch: any;
}

const MultibranchInfoCard: React.FC<
  MultibranchrInfoCardProps
> = ({ multibranch }) => {
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          <div className="rounded-full bg-verido-green-2 p-3 text-[16px] font-bold">
            {getInitials(multibranch?.name)}
          </div>
          <div>
            <h2 className="text-[22px] font-bold">{multibranch?.name}</h2>
            <p className="text-gray-text text-[14px]">{multibranch?.email}</p>
            <p
            ...
            >
              {multibranch?.status ? "Active" : "Inactive"}
            </p>
          </div>
        </div>
        ...
        <div className="flex px-6 py-6">
          <div className="w-1/2 flex flex-col gap-3">
            <p className="text-[14px] flex gap-2">
              ...
              <span className="font-normal">{multibranch?.name}</span>
            </p>
            <p className="text-[14px] flex gap-2">
              <span className="font-medium">Email Address:</span>
              <span className="font-normal">{multibranch?.email}</span>
            </p>
            <p className="text-[14px] flex gap-2">
              <span className="font-medium">User ID:</span>
              <span className="font-normal">{multibranch?.referenceId}</span>
            </p>
            <p className="text-[14px] flex gap-2">
              <span className="font-medium">Date Created:</span>
              <span className="font-normal">
                {formatDate(multibranch?.createdAt)}
              </span>
            </p>
          </div>
          {/* <div className="flex-1 flex flex-col gap-3">
            <p className="text-[14px] flex gap-2">
              ...
              <span className="font-normal">
                {multibranch?.superAgentDetails?.email}
              </span>
            </p>
            <p className="text-[14px] flex gap-2">
              <span className="font-medium">ID:</span>
              <span className="font-normal">
                {multibranch?.superAgentDetails?.referenceId}
              </span>
            </p>
            <p className="text-[14px] flex gap-2">
...
export default MultibranchInfoCard;
