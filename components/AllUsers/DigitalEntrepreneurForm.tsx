...
import useDigitalEntrepreneur from "@/lib/react-query/mutations/useDigitalEntrepreneur";
import { ICreateNewUsers } from "@/types";

import Image from "next/image";

interface DigitalEntrepreneurFormProps {
  closeModal: () => void;
}

const DigitalEntrepreneurForm = ({
  closeModal,
}: DigitalEntrepreneurFormProps) => {
  const { createDigitalEntrepreneurMutation } = useDigitalEntrepreneur();
  ...
  const onSubmit = async (payload: ICreateNewUsers) => {
    try {
      await createDigitalEntrepreneurMutation.mutateAsync(payload);
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        >
          {isSubmitting ? <LoadingSpinner /> : "Create Digital Entrepreneur"}
        </Button>
      </form>
    </>
  );
...
export default DigitalEntrepreneurForm;
