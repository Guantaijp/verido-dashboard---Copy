...
const DistibutorForm = ({ closeModal }: DistributorFormProps) => {
  const { createDistributorMutation } = useDistributors();
  ...
  const onSubmit = async (payload: ICreateNewUsers) => {
    try {
      await createDistributorMutation.mutateAsync(payload);
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        >
          {isSubmitting ? <LoadingSpinner /> : "Create Distributor"}
        </Button>
      </form>
    </>
  );
