...
import { Copy, MoreVertical, MoveLeft, MoveRight } from "lucide-react";
...
export function DigitalEntrepreneurTable<TData extends INewUsers, TValue>({
  columns,
  data,
  isFetching,
  onPageChange,
            ...
            <DropdownMenuItem
              ...
              onClick={(e) => {
                ...
                navigator.clipboard.writeText(data.referenceId).then(() => {
                  showToast(
                    "Success",
                    `Digital Entrepreneur Id copied to clipboard`,
                    "success"
                  );
                });
              }}
  ...
  const renderContent = () => {
      ...
      return (
        <TableRow className="text-sm font-bold text-gray-text">
          <TableCell colSpan={columns.length} className="h-24 text-center">
            No Digital Entrepreneurs found.
          </TableCell>
        </TableRow>
      );
    }
    return table.getRowModel().rows.map((row) => (
      <TableRow
        ...
        onClick={() => {
          const data = row.original;
          if (hasPermission("digital_entrepreneur.getSingle")) {
            router.push(`/digital-entrepreneur/${data._id}`);
          } else {
            showToast(
              "Access Denied",
    ...
    <div className="rounded-md">
      <div className="flex flex-col md:flex-row  gap-3 md:gap-0 justify-between mb-6">
        <div className="flex flex-col items-start gap-2">
          <h2 className="text-[24px] font-bold">Digital Entrepreneurs</h2>
        </div>
